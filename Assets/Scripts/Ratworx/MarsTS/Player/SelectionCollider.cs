...
namespace Ratworx.MarsTS.Player {
    ...
    public class SelectionCollider : MonoBehaviour {
		...
		private void OnTriggerStay (Collider other) {
			if (EntityCache.TryGetEntityComponent(other.attachedRigidbody.name, out ISelectable target)) {
				newlyHitUnits[other.attachedRigidbody.name] = target;
				target.Hover(true);
			}
		}
		...
		private void OnDestroy () {
			...
			foreach (ISelectable unit in factionList) {
				string[] splitKey = unit.Entity.RegistryType.Split(':');
			...
			// TODO: Make this dynamically pick the type with the most amount of units
