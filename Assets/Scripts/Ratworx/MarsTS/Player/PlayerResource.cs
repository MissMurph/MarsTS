...
namespace Ratworx.MarsTS.Player
{
    public class PlayerResource : NetworkBehaviour
    {
        ...
        private readonly NetworkVariable<int> _storedResources =
        ...
        private void Start() {
            if (NetworkManager.Singleton.IsServer) Amount = startingAmount;
        }
        ...
        private void OnResourceValueChange(int oldAmount, int newAmount) {
            _bus.PostGlobal(new ResourceUpdateEvent(_player, this));
        }
        ...
        [Rpc(SendTo.Server)]
        private void WithdrawServerRpc(int amount) => Withdraw(amount);
    }
}